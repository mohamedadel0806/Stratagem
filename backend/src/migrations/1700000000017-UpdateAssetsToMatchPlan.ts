...
export class UpdateAssetsToMatchPlan1700000000017 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Add foreign keys for business_units
    ...
    // Add indexes for business_units
    ...
    // Step 2: Create asset_category_enum and asset_types table
    ...
    // Step 3: Update physical_assets table
    // Add asset_type_id column
    ...
    // Add business_unit_id column (will migrate data later)
    await queryRunner.addColumn(
      'physical_assets',
      new TableColumn({
    ...
    // Change ip_addresses and mac_addresses to JSONB (check if columns exist first)
    ...
    // Change compliance_requirements to JSONB (check if column exists first)
    await queryRunner.query(`
      DO $$ BEGIN
    ...
    // Rename warranty_expiry_date to warranty_expiry if it exists
    await queryRunner.query(`
      DO $$ BEGIN
    ...
    // Create connectivity_status index only if column exists (handle both naming conventions)
    await queryRunner.query(`
      DO $$ 
      DECLARE
        col_name text;
    ...
    // GIN indexes for JSONB (only if columns exist and are JSONB)
    await queryRunner.query(`
      DO $$ 
      DECLARE
        col_name text;
    ...
    // Step 4: Update information_assets table
    await queryRunner.addColumn(
      'information_assets',
      new TableColumn({
    ...
    // Rename columns to match plan (handle both snake_case and camelCase)
    await queryRunner.query(`
      DO $$ BEGIN
    ...
    // Create reclassification_date index only if column exists (handle both naming conventions)
    await queryRunner.query(`
      DO $$ 
      DECLARE
        col_name text;
    ...
    // Step 5: Update business_applications table
    await queryRunner.addColumn(
    ...
    // Step 6: Update software_assets table
    await queryRunner.addColumn(
      'software_assets',
      new TableColumn({
        name: 'business_unit_id',
    ...
    // Create contract_end_date index only if column exists (handle both naming conventions)
    await queryRunner.query(`
      DO $$ 
      DECLARE
        col_name text;
