...
export class CreateRiskCategoriesTable1702000000001 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    // Create risk tolerance enum
    ...
    // Create indexes
    await queryRunner.createIndex('risk_categories', new TableIndex({ columnNames: ['code'] }));
    await queryRunner.createIndex('risk_categories', new TableIndex({ columnNames: ['parent_category_id'] }));
    await queryRunner.createIndex('risk_categories', new TableIndex({ columnNames: ['is_active'] }));
    await queryRunner.createIndex('risk_categories', new TableIndex({ columnNames: ['display_order'] }));
